...
func NewHandler(log *logger.Logger, usecase SessionUsecase) *Handler {
	return &Handler{
		log:     log,
		usecase: usecase,
	}
...
func (h *Handler) handleCreateSession(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.log.Error("failed to decode create session request: %v", err)
		writeError(w, http.StatusBadRequest, err)
		return
	}
	if req.TelegramID == 0 || req.Session == "" {
		err := errValidation("telegram_id and session_string are required")
		h.log.Warn("validation failed for create session: %v", err)
		writeError(w, http.StatusBadRequest, err)
		return
	}
	if req.LoginMethod == "" {
	...
	if err := h.usecase.CreateSession(r.Context(), input); err != nil {
		h.log.Error("usecase failed to create session for telegramID %d: %v", req.TelegramID, err)
...
func (h *Handler) handleDeleteSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Warn("failed to parse telegramID for delete session: %v", err)
		writeError(w, http.StatusBadRequest, errValidation("invalid telegramID"))
		return
	}
	if err := h.usecase.DeleteSession(r.Context(), id); err != nil {
		h.log.Error("usecase failed to delete session for telegramID %d: %v", id, err)
		writeError(w, http.StatusInternalServerError, err)
		return
	}
	w.Header().Set("Content-Type", "application/json")
...
func (h *Handler) handlePatchFeature(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Warn("failed to parse telegramID for patch feature: %v", err)
		writeError(w, http.StatusBadRequest, errValidation("invalid telegramID"))
		return
	}
	var payload featurePatchRequest
	if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
		h.log.Error("failed to decode patch feature request for telegramID %d: %v", telegramID, err)
		writeError(w, http.StatusBadRequest, err)
		return
	}
	...
	if err := h.usecase.UpdateFeature(r.Context(), feature, telegramID, payload.Payload); err != nil {
		h.log.Error("usecase failed to update feature '%s' for telegramID %d: %v", feature, telegramID, err)
		writeError(w, http.StatusInternalServerError, err)
		return
	}
	w.Header().Set("Content-Type", "application/json")
